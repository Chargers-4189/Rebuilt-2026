...
public class SinglePass extends SequentialCommandGroup {
  ...
  public SinglePass(Manager manager, SwerveSubsystem swerve, Vision vision, ChoreoTraj traj, boolean resetOdom, boolean withShooterTimeout) {
    ...
    addCommands(
      ...
      Commands.race(
        ...
        Commands.sequence(
          ...
          Commands.runOnce(() -> {
            vision.activate();
            manager.setState(RobotState.PRESPIN);
          })
        )
      ),
      Commands.runOnce(() -> vision.activate(), vision),
      new ScoreWithTaunt(manager, swerve, vision).withTimeout(withShooterTimeout ? AutoTable.kShooterTimeout.get() : 6),
