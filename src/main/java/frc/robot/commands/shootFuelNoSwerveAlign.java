...
public class shootFuelNoSwerveAlign extends Command {
  ...
  double shooterSpeed; //Speed of shooter
  ...
  @Override
  public void execute() {
    shooterSpeed = -1; //Lynn, you will determine this number through vision somehow
    ...
    if(Math.abs(angle - currentAngle) >= hoodTolerance){
      ...
      shooter.setIndexerPower(shooterSpeed); // I cannot see a time where we have the shooter running and we wouldnt want the indexer running. somebody please challenge this line I am not confident at all
      shooter.setShooterPower(shooterSpeed);
    }
